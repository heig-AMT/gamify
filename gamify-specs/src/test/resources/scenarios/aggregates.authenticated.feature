Feature: Validation of authenticated aggregates
  ...
  Scenario: I can get a category aggregate with two users
    Given I create the category payload cat with name catName
    And I PUT the cat payload to the api.categories.catName endpoint
    ...
    Given I create the event payload event1 with type rule1 and for user userId1
    Given I create the event payload event4 with type rule1 and for user userId1
    Given I create the event payload event2 with type rule2 and for user userId2
    Given I create the event payload event3 with type rule1 and for user userId3
    ...
    And The best user in the leaderboard response has 175 points
  ...
  Scenario: I can get a user aggregate with one event
    ...
    And I PUT the rule payload to the api.rules.ruleName endpoint
    Given I create the event payload event with type eventName and for user id
    When I POST the event payload to the api.events endpoint
    Then I GET from the api.users.id endpoint
    And I read the response payload
    And I receive a 200 status code

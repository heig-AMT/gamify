...
While we [load tested our system](load-testing/Add%20events.jmx), we realised that the `/events` endpoint sometimes skips point additions. We could not reproduce the issue with Cucumber tests, so we're glad to have found this issue with JMeter.
